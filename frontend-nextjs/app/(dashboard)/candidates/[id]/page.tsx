    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">候補者が見つかりません</h2>
          <p className="text-gray-600 mb-6">指定された候補者は存在しないか、アクセス権限がありません。</p>
          <button
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">候補者詳細</h1>
              <p className="text-gray-600 mt-1">ID: {candidate.rirekisho_id || candidate.id}</p>
            </div>
          </div>
        ...
        {isVisaExpiringSoon && (
          <div className="mb-6 bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-r-lg">
            <div className="flex">
              ...
              <div>
                <h3 className="text-sm font-medium text-yellow-800">在留期間満了が近づいています</h3>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-6">
              <div className="flex flex-col items-center">
                ...
                {candidate.full_name_kana && (
                  <p className="text-gray-600 mb-2">{candidate.full_name_kana}</p>
                )}

                {candidate.full_name_roman && (
                  <p className="text-gray-500 text-sm mb-4">{candidate.full_name_roman}</p>
                )}
                ...
                {candidate.age && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-900 font-medium">{candidate.age}歳</p>
                    </div>
                  </div>
                )}
                ...
                {candidate.nationality && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-900 font-medium">{candidate.nationality}</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-semibold text-gray-900">在留資格・書類情報</h3>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {candidate.residence_status && (
                  <div>
                    <p className="text-sm text-gray-500 mb-1">在留資格</p>
                    <p className="text-gray-900 font-medium">{candidate.residence_status}</p>
                  </div>
                )}

                {candidate.visa_period && (
                  <div>
                    <p className="text-sm text-gray-500 mb-1">在留期間</p>
                    <p className="text-gray-900">{candidate.visa_period}</p>
                  </div>
                )}

                {candidate.residence_expiry && (
                  <div>
                    <p className="text-sm text-gray-500 mb-1">在留期間満了日</p>
                    <p className={`font-medium ${isVisaExpiringSoon ? 'text-yellow-600' : 'text-gray-900'}`}>
                ...
                {candidate.residence_card_number && (
                  <div>
                    <p className="text-sm text-gray-500 mb-1">在留カード番号</p>
                    <p className="text-gray-900 font-mono">{candidate.residence_card_number}</p>
                  </div>
                )}

                {candidate.passport_number && (
                  <div>
                    <p className="text-sm text-gray-500 mb-1">パスポート番号</p>
                    <p className="text-gray-900 font-mono">{candidate.passport_number}</p>
                  </div>
                )}

                {candidate.passport_expiry && (
                  <div>
                    <p className="text-sm text-gray-500 mb-1">パスポート期限</p>
                    <p className="text-gray-900">{new Date(candidate.passport_expiry).toLocaleDateString('ja-JP')}</p>
                  </div>
                )}

                {candidate.license_number && (
                  <div>
                    <p className="text-sm text-gray-500 mb-1">運転免許証番号</p>
                    <p className="text-gray-900 font-mono">{candidate.license_number}</p>
                  </div>
                )}

                {candidate.license_expiry && (
                  <div>
                    <p className="text-sm text-gray-500 mb-1">運転免許期限</p>
                    <p className="text-gray-900">{new Date(candidate.license_expiry).toLocaleDateString('ja-JP')}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {(candidate.listening_level || candidate.speaking_level || candidate.reading_level || candidate.writing_level) && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900">日本語能力</h3>
                </div>

                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {candidate.listening_level && (
                    <div>
                      <p className="text-sm text-gray-500 mb-1">聞く</p>
                      <p className="text-gray-900 font-medium">{candidate.listening_level}</p>
                    </div>
                  )}

                  {candidate.speaking_level && (
                    <div>
                      <p className="text-sm text-gray-500 mb-1">話す</p>
                      <p className="text-gray-900 font-medium">{candidate.speaking_level}</p>
                    </div>
                  )}

                  {candidate.reading_level && (
                    <div>
                      <p className="text-sm text-gray-500 mb-1">読む</p>
                      <p className="text-gray-900 font-medium">{candidate.reading_level}</p>
                    </div>
                  )}

                  {candidate.writing_level && (
                    <div>
                      <p className="text-sm text-gray-500 mb-1">書く</p>
                      <p className="text-gray-900 font-medium">{candidate.writing_level}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {hasExperience && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900">経験作業内容</h3>
                </div>

                <div className="flex flex-wrap gap-2">
                  {experienceFields.map(field => (
                    candidate[field.key as keyof Candidate] && (
                      <span key={field.key} className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-blue-100 text-blue-800">
            ...
            {(candidate.emergency_contact_name || candidate.emergency_contact_phone) && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900">緊急連絡先</h3>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {candidate.emergency_contact_name && (
                    <div>
                      ...
                      <p className="text-gray-900 font-medium">{candidate.emergency_contact_name}</p>
                    </div>
                  )}

                  {candidate.emergency_contact_relation && (
                    <div>
                      <p className="text-sm text-gray-500 mb-1">続柄</p>
                      <p className="text-gray-900">{candidate.emergency_contact_relation}</p>
                    </div>
                  )}
                  ...
                  {candidate.emergency_contact_phone && (
                    <div>
                      <p className="text-sm text-gray-500 mb-1">電話番号</p>
                      <p className="text-gray-900">{candidate.emergency_contact_phone}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-gray-50 rounded-2xl border border-gray-200 p-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">システム情報</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                {candidate.created_at && (
                  <div>
                    <p className="text-gray-500">登録日時</p>
                    <p className="text-gray-700">{new Date(candidate.created_at).toLocaleString('ja-JP')}</p>
                  </div>
                )}

                {candidate.updated_at && (
                  <div>
                    <p className="text-gray-500">更新日時</p>
                    <p className="text-gray-700">{new Date(candidate.updated_at).toLocaleString('ja-JP')}</p>
                  </div>
                )}
              </div>
            </div>
