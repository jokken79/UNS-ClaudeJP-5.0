...
export default function Page() {
  ...
  const [isGenerating, setIsGenerating] = useState<number | null>(null); // Index of the job being generated
 ...
 // --- End Handlers ---
    ...
    try {
      ...
      for (let i = 0; i < 5; i++) { // Retry up to 5 times
        ...
        if (response.status === 429 || response.status >= 500) { // Throttling or server error
          // console.log(`API call failed with status ${response.status}. Retrying in ${delay}ms...`); // Console log removed as per instructions for retries
      ...
      return "エラーが発生しました。後でもう一度お試しください。"; // Error occurred. Please try again later.
    }
  };

  /**
   * ✨ Generates a job description for a specific role using Gemini.
   */
  const handleGenerateJobDescription = async (index: number) => {
    const job = data.jobs[index];
    const company = job.hakensaki || job.hakenmoto;
    const role = job.content; // User should input a job title or keywords here first

    if (!role) {
      // Use a custom message box instead of alert
       setReviewResult("先ずは内容の欄に役職やキーワードを入力してください。"); // Changed 作業内容 to 内容
       setIsReviewModalOpen(true);
      return;
    }

    setIsGenerating(index);
    const prompt = `履歴書に記載するため、[${company}]での職務「${role}」について、プロフェッショナルな日本語の職務内容を80文字以内で簡潔に作成してください。`;
    const generatedContent = await callGeminiAPI(prompt);
    if (generatedContent && !generatedContent.startsWith("エラーが発生しました")) {
      updateJob(index, { content: generatedContent.replace(/"/g, '') });
    } else if (generatedContent) {
        setReviewResult(generatedContent); // Show error in modal
        setIsReviewModalOpen(true);
    }
  ...
  /**
   * ✨ Reviews the entire resume using Gemini and provides feedback.
   */
  const handleAiReview = async () => {
    setIsReviewing(true);
    setReviewResult(""); // Clear previous results immediately
    setIsReviewModalOpen(true); // Open modal to show loading state
    const prompt = `あなたは日本のプロのキャリアアドバイザーです。以下の履歴書データをレビューし、改善のための具体的な提案を3つ、日本語で箇条書きにしてください。明確さ、プロフェッショナリズム、そしてインパクトに焦点を当ててください。履歴書データはJSON形式です：\n\n${JSON.stringify(data)}`;
    const review = await callGeminiAPI(prompt);
    setReviewResult(review || "レビューの生成中にエラーが発生しました。"); // Set result or error message
    ...
    <div id="rirekisho-page-root" className="mx-auto max-w-[1200px] px-4 py-6 print:p-0 font-noto-sans-jp">
      ...
      <div className="sticky top-0 z-10 mb-6 flex items-center justify-center gap-3 rounded-xl border bg-white/80 p-3 shadow-md backdrop-blur print:hidden">
        ...
        <button
          ...
          className="rounded-lg bg-purple-600 px-4 py-2 font-semibold text-white hover:bg-purple-700 disabled:bg-purple-300 flex items-center gap-2"
        >
          {isReviewing ? 'レビュー中...' : '✨ AI レビュー'}
        </button>
      </div>
                    ...
                    <td className="border border-black px-2 py-1"><input value={row.hakensaki} onChange={(e) => updateJob(i, { hakensaki: e.target.value })} className="w-full border-0 p-0 outline-none" /></td>
                     ...
                     <td className="border border-black px-2 py-1"> {/* Moved Cell: 内容 (Content) */}
                       <div className="flex items-center gap-1">
                         ...
                         <button onClick={() => handleGenerateJobDescription(i)} disabled={isGenerating === i} className="p-1 rounded-md hover:bg-gray-100 disabled:opacity-50 print:hidden">
                            {isGenerating === i ? '...' : '✨'}
                         </button>
                       </div>
                     </td>
      ...
      {/* AI Review Modal */}
      {isReviewModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 print:hidden" onClick={() => setIsReviewModalOpen(false)}>
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-2xl w-full" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4">{isReviewing ? 'レビュー生成中...' : '✨ AIによるレビュー結果'}</h3>
            <div className="prose prose-sm max-w-none whitespace-pre-wrap min-h-[100px]">
              {isReviewing ? <div className="flex justify-center items-center h-full"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-500"></div></div> : (reviewResult || "レビューの生成中にエラーが発生しました。")}
            </div>
            <button
              onClick={() => setIsReviewModalOpen(false)}
              disabled={isReviewing}
              className="mt-6 rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white hover:bg-blue-700 disabled:bg-blue-300"
            >
              閉じる
            </button>
          </div>
