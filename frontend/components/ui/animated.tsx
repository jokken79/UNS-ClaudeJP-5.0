...
export const AnimatedDiv = React.forwardRef<HTMLDivElement, AnimatedDivProps>(
  ({ variant = 'fadeIn', delay = 0, className, children, ...props }, ref) => {
    const variants = variant === 'fadeInUp' ? fadeInUp : variant === 'fadeIn' ? fadeIn : undefined;
    ...
    return (
      <motion.div
        ...
        variants={variants ? getVariants(variants) : undefined}
  ...
  ({ staggerDelay = 0.1, className, children, ...props }, ref) => {
    ...
    return (
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        variants={getVariants(customStagger)}
        {...props}
      >
        {children}
      </motion.div>
